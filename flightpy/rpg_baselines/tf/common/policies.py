...
#
from rpg_baselines.tf.common.distributions import make_proba_dist_type, CategoricalProbabilityDistribution, \
    MultiCategoricalProbabilityDistribution, DiagGaussianProbabilityDistribution, BernoulliProbabilityDistribution
...
def nature_cnn(scaled_images, **kwargs):
    ...
    layer_2 = activ(conv(layer_1, 'c2', n_filters=64,
                         filter_size=4, stride=2, init_scale=np.sqrt(2), **kwargs))
...
def mlp_extractor(flat_observations, net_arch, act_fun):
    ...
    for idx, layer in enumerate(net_arch):
        if isinstance(layer, int):  # Check that this is a shared layer
            layer_size = layer
            latent = act_fun(linear(latent, "shared_fc{}".format(
                idx), layer_size, init_scale=np.sqrt(2)))
        else:
            assert isinstance(
    ...
    for idx, (pi_layer_size, vf_layer_size) in enumerate(zip_longest(policy_only_layers, value_only_layers)):
        ...
        if vf_layer_size is not None:
            ...
            latent_value = act_fun(linear(latent_value, "vf_fc{}".format(
                idx), vf_layer_size, init_scale=np.sqrt(2)))
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            self._tanh_deterministic_action = tf.tanh(
                self._deterministic_action)
            self._neglogp = self.proba_distribution.neglogp(self.action)
            self._tanh_neglogp = self.proba_distribution.tanh_neglogp(
                self.action)
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
        ...
        with tf.variable_scope("input", reuse=False):
            self._dones_ph = tf.placeholder(
                tf.float32, (n_batch, ), name="dones_ph")  # (done t-1)
            ...
            with tf.variable_scope("model", reuse=reuse):
                ...
                for idx, layer in enumerate(net_arch):
                    if isinstance(layer, int):  # Check that this is a shared layer
                        ...
                        input_sequence = batch_to_seq(
                            latent, self.n_env, n_steps)
                        masks = batch_to_seq(
                            self.dones_ph, self.n_env, n_steps)
                        rnn_output, self.snew = lstm(input_sequence, masks, self.states_ph, 'lstm1', n_hidden=n_lstm,
                                                     layer_norm=layer_norm)
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            self._proba_distribution, self._policy, self.q_value = \
                self.pdtype.proba_distribution_from_latent(
                    pi_latent, vf_latent, init_scale=1)
