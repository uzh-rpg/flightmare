"""
Following changes have been made by Yunlong:

    1) Add Tanh activation to Policy Net
    2) Reset all environment after collecting enough rollouts
"""
import os
import time
...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if self.use_tanh_act:
            self.policy.action_net = th.nn.Sequential(self.policy.action_net,
                                                      th.nn.Tanh())
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            rollout_buffer.add(obs_norm, actions, rewards,
        ...
        # 2) Added by Yunlong: Reset all environment after collecting enough rollouts
        # It helps collecting more useful data for training (my conjecture).
        # self._last_obs, infos = env.reset_and_update_info()
        # self._update_info_buffer(infos)
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if log_interval is not None and iteration % log_interval[0] == 0:
                ...
                for i in range(self.env.rew_dim-1):
                    logger.record(
                        ...
                        safe_mean([ep_info[self.env.reward_names[i]] for ep_info in self.ep_info_buffer]))
            ...
            if log_interval is not None and iteration % log_interval[1] == 0:
                policy_path = logger.get_dir() + "/Policy"
                if not os.path.exists(policy_path):
                    os.mkdir(policy_path)
                # save the model parameters
                self.policy.save(
                    policy_path + "/iter_{0:05d}.pth".format(iteration))

                self.env.save_rms(save_dir=logger.get_dir() +
                                  "/RMS", n_iter=iteration)
            #     self.eval()
