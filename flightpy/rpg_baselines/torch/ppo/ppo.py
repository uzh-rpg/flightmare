...
class PPO(OnPolicyAlgorithm):
    ...
    def eval(self) -> None:
        # if not os.path.exists(logger.get_dir() + "/Eval"):
        #     os.makedirs(logger.get_dir() + "/Eval")

        # n_step, max_step = 0, 100
        # last_obs, infos = self.eval_env.reset_and_update_info()
        # while n_step < max_step and self.eval_env is not None:
        #     # Normalize observation (for RacingEnv)
        #     obs_mean, obs_std = self.eval_env.get_obs_norm()
        #     obs_norm = (last_obs - obs_mean) / obs_std

        #     act, _ = self.predict(
        #         obs_norm, deterministic=True)

        #     new_obs, rewards, dones, infos = self.eval_env.step(act)
        #     n_step += 1
        #     print(dones)
        pass
